...
export async function updateCard(boardDir, cardId, updates) {
    const { board } = await readBoard(boardDir);
    ...
    await writeFileAtomic(join(boardDir, 'cards', `${cardId}.md`), serializeCardMarkdown(updated));
    return updated;
}
export async function moveCard(boardDir, cardId, targetColumn, position) {
    const { board } = await readBoard(boardDir);
    ...
    await writeFileAtomic(join(boardDir, 'cards', `${cardId}.md`), serializeCardMarkdown(card));
    await writeBoardYaml(boardDir, board);
    return card;
}
export async function appendNote(boardDir, cardId, note) {
    const card = await readCard(boardDir, cardId);
    if (!card) {
        throw new CardNotFoundError(cardId);
    }
...
export async function deleteCard(boardDir, cardId) {
    const { board } = await readBoard(boardDir);
    const card = await readCard(boardDir, cardId);
    if (!card) {
        throw new CardNotFoundError(cardId);
...
export async function archiveCard(boardDir, cardId) {
    const { board } = await readBoard(boardDir);
    const card = await readCard(boardDir, cardId);
    if (!card) {
        throw new CardNotFoundError(cardId);
    ...
    await rename(join(boardDir, 'cards', `${cardId}.md`), join(archiveDir, `${cardId}.md`));
    await writeBoardYaml(boardDir, board);
}
//# sourceMappingURL=card-writer.js.map